...
public class CalcActivity extends Activity {
	...
	private int[] btNum = { R.id.txtspl, R.id.txt0, R.id.txt1, R.id.txt2,
			R.id.txt3, R.id.txt4, R.id.txt5, R.id.txt6, R.id.txt7, R.id.txt8,
			R.id.txt9 };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < btNum.length; i++) {
			System.out.println("----d-ddd-------df==============" + btNum[i]);
	...
	// Field [] butNum = R.id.class.getDeclaredFields();
	// Button [] buts = new Button[butNum.length] ;
	// TableLayout tableLayout = (TableLayout)
	// super.findViewById(R.id.tablelayout01) ;
	// GetNumber get = new GetNumber() ;
	// int [] bu = new int[butNum.length] ;
	//
	// for (int i = 0; i < butNum.length; i++) {
	// if(butNum[i].getName().startsWith("txt")){
	// System.out.println("===========sdfwerwee============"+buts[i]);
	// String ss = butNum[i].getName().toString() ;
	//
	// System.out.println("===========dddddsdfsadfsdfsdfdddd============"+bu[i]);
	// buts[i] = (Button) findViewById(bu[i]) ;
	// buts[i].setOnClickListener(get) ;
	// }
	//
	// }
	// }
	public void Test() {
		...
		// (Button) this.findViewById(getResources().getIdentifier("am" + i,
		// "id", getPackageName()));
	}
